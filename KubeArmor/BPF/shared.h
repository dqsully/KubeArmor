...
#define RULE_KEY_STR_LEN 256 // String length inside a rule key

// Rule types
#define RULE_TYPE_FILE       1
#define RULE_TYPE_PROCESS    2
#define RULE_TYPE_NETWORK    3
#define RULE_TYPE_CAPABILITY 4
...
// Path rule u32 bitwise offsets (a rule is 4 bits)
...
// source executable.
//
// Surprisingly, the inner map spec can't be defined here in C, only the code
...
struct network_rule_key {
  union {
    ...
    // field, and should be <32 (the first ASCII printable character) so that it
    // absolutely cannot clash with a path rule.
    struct {
      // The non-path rule type
      char rule_type;
      ...
      // eBPF won't let us read this 3rd byte until we write to it first, so we
      // still ned to zero it out ourselves at some point.
      char null_byte;
    };
    char raw[3];
  };
...
// struct capability_rule_key is a pseudo-path rule key for capability keys. See
// struct network_rule_key for more info.
struct capability_rule_key {
  union {
    struct {
      // The non-path rule type
      char rule_type;

      // The capability number (e.g. CAP_SYS_ADMIN, CAP_NET_BIND_SERVICE, etc.)
      char capability;
...
// 512 bytes
struct rule_key {
  // The path to the target file (being executed, read, written, ect.)
...
struct event {
  ...
  // Linux's recorded command line
  u8 comm[TASK_COMM_LEN];

  // Rule key and flags used to match the rule for this event
  //
  // KubeArmor understand that opening a file at /foo/bar/baz would match a
...
// get task_pid_ns_id() returns the Linux PID namespace for a given task.
...
// get_vfsmount_ns_id() returns the Linux mount namespace for a given vfsmount.
static __always_inline u32 get_vfsmount_ns_id(struct vfsmount *mnt) {
  return BPF_CORE_READ(real_mount(mnt), mnt_ns, ns).inum;
}

// get_vfsmount_ns_root() returns the root mount (e.g. `/`) for a given
// vfsmount's namespace.
...
// get_task_exe_mnt_ns_id() returns the Linux mount namespace ID for a given
// task's executable file (e.g. where the executable file was originally run
// from).
...
// is_task_exe_external() returns true if the task's executable file is in a
// different namespace than the task's current mount namespace (e.g. if the task
// entered a container's mount namespace).
...
// is_path_external() returns true if the given path is in a different namespace
// than the task's current mount namespace.
...
static char *prepend_path(struct path *path, int buf_idx) {
  ...
  // Load the buffer to write into
  struct buffer *buf = bpf_map_lookup_elem(&buffers, &buf_idx);
  if (buf == NULL) {
    return NULL;
  }

  // The eBPF verifier thinks buf->prepend_offset could be anything, so by
  // bitwise ANDing it and storing it as a variable, we prove to the verifier
  // that it must be within the bounds of the buffer, and we never have to
  // bounds-check it again (unless LLVM's optimizer gets too clever).
  u32 offset = buf->prepend_offset & BUFFER_HALF_MASK;

  // Char to prepend to the final path.
  //
  // Every Linux path starts with '/', but to save eBPF instruction complexity,
  // we may overwrite path_start to '?' to output an 'unknown' path instead.
  char path_start = '/';

  // dentry (directory entry) for the current path
  struct dentry *dentry = path->dentry;
  // mount for the current path
  struct mount *mnt = real_mount(path->mnt);
  // dentry for the top of the mount
  ...
  for (int i = 0; i < 30; i++) {
    if (dentry == mnt_root) {
      ...
      // grab the parent mount, get the dentry of that mount, and continue
    ...
    // won't ever be >255 for a dentry. (In this case we use BUFFER_HALF_MASK
    // for some eBPF trickery explained below)
    ...
    //
    // This is where a memory-mapped runc executable for the `runc init` command
    // (used to start or exec into a container) would show up as a '?' in any
    // alerts, because the file's name is unknowable unless we pre-cached it.
    //
    // TODO: track mmap'd files in another map to resolve this issue
    if (d_len == 0) {
      offset = buf->prepend_offset - 1;
      path_start = '?';
      goto save_path;
    ...
    //
    // There is a HUGE hack here to please the eBPF verifier. Even though we
    // know that d_len will only be between 0 and 255, if we let the verifier
    // know that, it will track every possible maximum offset written to the
    // buffer and returned. Instead, we tell the verifier that d_len is up to
    // 50% of the buffer's size, which in the following function call, means (to
    // the verifier) that:
    //
    ...
    //   buffer (the verifier can't track the relationship between offset and
    //   d_len, just their individual bounds)
    ...
    //
    // This means that each loop looks essentially the same to the verifier so
    // it doesn't have to revalidate the same instructions as much. When tested
    // with the loop running only 5 times, this single hack reduced the
    ...
    // If there was an error reading, return NULL
    if (sz < 0) {
      return NULL;
    }
...
save_path:
  ...
  // Write a null terminator at the end of the path so that we have a
  // null-terminated string again. (We already confirmed at the start of this
  // function that buf->prepend_offset is >= 1, but that wasn't saved to the
  // stack, and it's not in any registers anymore, so we have to prove to the
  // eBPF verifier that it's within bounds again.)
...
// prepend_ext() prepends the '$' character to the prepend side of the buffer,
// indicating that the path was external to the container.
...
static __always_inline u32 match_dir(
  void *inner,
  char *path,
  struct rule_key *key,
  u32 mask
  ...
  // /foo/bar/baz/ (for some file /foo/bar/baz/file.txt) and file a rule there,
  ...
  // NULL, it's the most-specific rule and will take priority. If /foo/bar is as
  // far as the policies go, then /foo/bar/baz/ will be NULL and any
  // previously-saved recursive rule will apply instead.
  //
  // >50% of all program complexity is caused by this loop, so any small state
  // or instruction optimizations can really go a long way.
  //
  // Also, while this loop can be unrolled for an overall ~23% instruction
  // complexity improvement, it also doubles the binary size and increases total
  // verification time by an extra 15%, so it's not worth it.
  for (int i = 0; i < RULE_KEY_STR_LEN - 1; i++) {
    ...
    // Write out the now-deeper path that we're checking to our key buffer.
    //
    // Yes, this 'needlessly' overwrites bytes in the key that were already
    // written, but this way is much simpler for eBPF to verify.
    ...
    if (*dir_rule & mask & RULE_MASK_ANY_RECURSIVE) {
      if (*dir_rule & mask & RULE_MASK_ANY_HINT) {
        ...
        //
        // Pre-masking this value saves some instructions
  ...
  // If the last directory in the path matched a rule, return that since it's
  // the most specific rule possible. This is for both recursive and
  // non-recursive policies.
  if (dir_rule != NULL && (*dir_rule & mask) != RULE_NONE) {
    return *dir_rule & mask;
  }
...
// apply_rule() determins the return value for a given event, and submits it to
...
static __always_inline int apply_rule(
  ...
  // If there is no default rule or if the rule is an allow rule, return ok and
  ...
  // Fill in the path and/or source if they were provided
  if (path != NULL) {
...
static __always_inline u32 match_net(
  ...
  if (gp_key->path[1] != 0) {
    ...
    // someone wants a 'most-general' network rule for a container that isn't
    // the default posture. File/process rules have an equivalent already
    // anyways: a recursive directory rule at /.
    gp_key->path[1] = 0;
...
// match_and_enforce_path_hooks_write() matches the given path, event ID, and
// event write status with the enforcement rules for the current container, and
// enforces the best-matching rule, returning the appropriate LSM hook return
// value.
//
// Because eBPF is weird, callers of this function MUST NOT pass a struct f_path
// * received from an LSM hook directory. Instead, construct a new struct path
//   on the stack and read at least the `mnt` property using BPF_CORE_READ (or
//   bpf_probe_read), and then pass the pointer to that struct. This erases
//   certain information about the pointer types, and keeps pointer types
//   consistent further within the eBPF program.
static __always_inline int match_and_enforce_path_hooks_write(
  ...
  // so far I've noticed al ot of expected cross-namespace file accesses that
  ...
  if (is_path_external(f_path, task)) {
    return 0;
  }
  ...
  // (guaranteed to be within the container's namespace)
  ...
  // to cover FILE_READ_BY_OWNER and FILE_WRITE_BY_OWNER
